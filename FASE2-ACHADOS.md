...
- `ClaudeQuirks.strip_trailing_tag_leak` deve ser considerado (e testado de novo) em todo campo de texto longo gerado por `claude` em Discovery/Delivery — o vazamento não é específico da Wiki, é do backend. **Feito na Fase 3:** aplicado em `der.diagrama_mermaid` (o único campo de string solto na última posição de um schema de Discovery, mesmo padrão estrutural do vazamento original) — não reproduzido nas 9 chamadas reais de Discovery desta fase (brief/atributos/requisitos/adr/der/diagramas/features/historias×2), mas mantido como precaução já que a causa raiz nunca foi confirmada.
