...
- Evitar strings mágicas repetidas (nomes de `action`, `type`, categorias de wiki) — quando o mesmo literal aparece em mais de um ponto de decisão, extrair para uma constante nomeada ou, quando o valor for fechado, considerar `enum` (ver limitação registrada em [MHL-Melhorias.md](MHL-Melhorias.md)).
...
- Os achados de sintaxe da Fase 1 (FASE0-ACHADOS.md §8) — `acc += [x]` em vez de `acc.append(x)`, `obj["campo"] = x` para escrita de campo, qualificar chamada a método-irmão de `tool` — viram convenção padrão de todo `.mh` novo, não correções pontuais.
